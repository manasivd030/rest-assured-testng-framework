...
public class AwaitilityTests {
    ...
    public void waitUntilAsserted() {
        ...
        Awaitility.await()
                  .and().with().alias("waitUntilAsserted")
                  .and().with().timeout(Duration.ofSeconds(5))
                  .then().untilAsserted(() -> {
                      getBookingApi.getBookingById(125)
...
//                                   .then().assertThat().statusCode(400);
    ...
    public void waitUntil() {
        ...
        Awaitility.await()
                  .and().with().alias("waitUntil")
                  .and().with().timeout(Duration.ofSeconds(5))
                  .then().until(() -> {
                      var statusCode = getBookingApi.getBookingById(125).statusCode();
    ...
    public void waitUntilAndIgnoreAllExceptions() {
        ...
        Awaitility.await()
                  .and().with().alias("waitUntilAndIgnoreAllExceptions")
                  .and().with().timeout(Duration.ofSeconds(5))
    ...
    public void waitUntilAndIgnoreSpecificException() {
        ...
        Awaitility.await()
                  .and().with().alias("waitUntilAndIgnoreSpecificException")
                  .and().with().timeout(Duration.ofSeconds(5))
    ...
    public void definePollingDelay() {
        ...
        Awaitility.await()
                  .and().with().alias("definePollingDelay")
                  .and().with().timeout(Duration.ofSeconds(15))
                  .and().ignoreExceptionsInstanceOf(AssertionError.class)
                  .and().pollDelay(Duration.ofMillis(200))
                  .then().until(() -> {
                      getBookingApi.getBookingById(125)
                                   .then().assertThat().statusCode(200);
                      return true;
